...

{
  config = lib.mkIf config.services.mpd.enable {
    preferences.mediaplayer = lib.mkDefault "ncmpcpp";
    programs.ncmpcpp = {
      enable = true;
      settings = {
    ...
    xdg.configFile = {
      "ncmpcpp/get_and_copy_purl_from_current_song.sh" = {
        text = ''
          ...
          current_song_file_path="${config.services.mpd.musicDirectory}/$(${lib.getExe' pkgs.mpc "mpc"} current -f %file%)"
          ...
          if [ -n "$purl" ]; then
            ...
            ${lib.getExe' pkgs.libnotify "notify-send"} "ncmpcpp: PURL copied to clipboard" "$purl"
          fi
        '';
        executable = true;
      };
      "ncmpcpp/delete_current_song.sh" = {
        text = ''
          ...
          ${lib.getExe' pkgs.trash-cli "trash"} -f "$current_song_file_path" && {
            ${lib.getExe' pkgs.libnotify "notify-send"} "ncmpcpp: Song moved to trash" "Moved $current_song_file_path to trash"
          } || {
            ${lib.getExe' pkgs.libnotify "notify-send"} "ncmpcpp: Failed to delete current song" "Could not delete $current_song_file_path"
          }
          ${lib.getExe' pkgs.mpc "mpc"} update >/dev/null 2>&1
          ${lib.getExe' pkgs.mpc "mpc"} next >/dev/null 2>&1
        '';
        executable = true;
      };
    };
