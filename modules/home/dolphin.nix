...
let
  dolphinrc = ''
    ...
    Version=202
    ViewPropsTimestamp=2026,5,18,9,12,24.85
...
in
{
  home.activation.dolphinToolbar = lib.hm.dag.entryAfter [ "writeBoundary" ] ''
    target_ui_rc="${config.xdg.dataHome}/kxmlgui5/dolphin/dolphinui.rc"
    mkdir -p "$(dirname "$target_ui_rc")"
    cat >"$target_ui_rc" <<'EOF'
    ${dolphinui}
    EOF
    cat >"${config.xdg.configHome}/dolphinrc" <<'EOF'
    ${dolphinrc}
    EOF
  '';
}
