...
in
{
  programs.tmux = {
    ...
    extraConfig = ''
      ...
      #~ Keep only each managed session's initial pane alive; split panes must close normally.
  ...
  xdg.configFile = {
    "tmux/omt.conf".source = ./tmux/omt.conf;
    "tmux/omt.conf.local".source = ./tmux/omt.conf.local;
  };
}
