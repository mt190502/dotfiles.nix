...

{
  wayland.windowManager.sway.config = {
    startup = [
      ...
      {
        command =
          "${pkgs.swayidle}/bin/swayidle -w timeout 120 '~/.config/sway/scripts.d/powermenu.sh --lock' timeout 140 'swaymsg output * dpms off' resume 'swaymsg output * dpms on'";
      }
      { command = "${pkgs.tmux}/bin/tmux new-session -ds daemonmodetmux"; }
      {
        command =
          "${pkgs.wl-clipboard}/bin/wl-paste -w $HOME/scripts/cliphist store";
      }
